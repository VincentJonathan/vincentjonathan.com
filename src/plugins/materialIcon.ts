...
export function getIcon(fileName: string): string {
  ...
  const fileExtensionTemp = fileName.split(/\./g);
  fileExtensionTemp.shift();
  const fileExtension = fileExtensionTemp.join(".");

  for (const fileIcon of fileIcons.icons) {
    if (fileIcon.fileExtensions?.includes(fileExtension.toLowerCase())) {
      iconName = fileIcon.name;
    }
    if (fileIcon.fileNames?.includes(fileName.toLowerCase())) {
      iconName = fileIcon.name;
      break;
    }
  }
